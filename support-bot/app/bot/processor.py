    ...
    async with get_lock(user_id):
        ...
        if planning.is_conversational:
            ...
            if rerank.enough_context:
                ...
                context_prefix = (
                    "Контекст из базы знаний отобран внутренним RAG pipeline, но он ПОМЕЧЕН КАК НЕПОЛНЫЙ — "
                    "прямого ответа в нём, скорее всего, нет. "
                    "Отвечай только по этим фрагментам и только в той части, которую они прямо покрывают. "
                    "Не придумывай шаги, варианты или места интерфейса, которых нет в контексте. "
                    "ВАЖНО: если фрагменты относятся к ДРУГОЙ теме или функции, чем спрашивает пользователь "
                    "(например, пользователь спрашивает про интеграцию/функцию/сервис, которого в контексте нет — "
                    "1С, выгрузку, стороннюю систему, отдельную возможность, — а фрагменты лишь про общий экспорт "
                    "или другой раздел), НЕ натягивай их на вопрос и НЕ описывай функцию, которой нет в контексте. "
                    "В таком случае, а также если фрагменты не дают прямого ответа, вызови transfer_to_operator "
                    "и ничего не выдумывай.\n"
                )
