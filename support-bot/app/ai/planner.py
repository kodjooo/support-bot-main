...
- Вопросы про расхождение или несовпадение данных ("не сходятся", "не совпадает", "расхождение", "разница", "сверка") зависят от маркетплейса: причины расхождений и отчёты для сверки у Wildberries и Ozon разные. Если в таком вопросе НЕ назван маркетплейс (нигде нет Wildberries/WB/ВБ/Wildberries/Ozon/Озон) и не указано явно, что сравнение идёт именно с данными внутри Sellerdata, верни need_clarification с коротким вопросом вида "С чем именно не сходятся данные — с Wildberries, с Ozon или с данными в Sellerdata?". Не подставляй маркетплейс по умолчанию и не отвечай сразу по обоим. Если маркетплейс в вопросе уже назван (или назван в pending-уточнении) — верни ready и не спрашивай его повторно.
