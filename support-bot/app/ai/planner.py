...
class PlanningResult:
    ...
    def is_conversational(self) -> bool:
        """Реплика, не требующая поиска в базе знаний: planner вернул ready
        с пустым search_query. Сюда попадают и разговорные реплики (приветствие,
        благодарность, подтверждение), и follow-up, на который можно ответить
        из контекста предыдущего ответа без нового поиска. В обоих случаях
        processor отправляет сообщение прямо ассистенту с last_response_id."""
...
Во входных данных может быть recent_dialogue с предыдущим вопросом пользователя (last_user_text) и предыдущим ответом бота (last_bot_answer). Это контекст диалога — используй его, чтобы понять текущее сообщение:
- Если текущее сообщение — follow-up к предыдущему ответу (содержит отсылки "это", "здесь", "тогда", "т.е.", "а если", "правильно понимаю", уточнение или переспрашивание по уже сказанному), трактуй его в контексте предыдущей темы, а не как отдельный вопрос.
- Если на такой follow-up можно ответить, просто опираясь на предыдущий ответ бота (переспрашивание, просьба подтвердить или перефразировать уже сказанное, и для этого НЕ нужны новые факты из базы) — верни status=ready с пустым search_query. Финальный ассистент ответит из контекста диалога.
- Если follow-up требует НОВЫХ фактов из базы знаний (другая деталь той же темы, которой не было в предыдущем ответе), верни status=ready с НЕпустым search_query и обязательно включи в него тему из предыдущего вопроса/ответа. Например, если ранее речь шла про выкупаемость, а пользователь спрашивает "а заказы в пути там учитываются?", search_query должен явно содержать "выкупаемость", а не только "заказы в пути".
- Не теряй тему: не строй search_query так, будто follow-up — это вопрос на пустом месте.
...
- search_query должен быть близок к формулировке пользователя. Сохраняй ключевые слова из вопроса как есть (например "восклицательный знак", "иконка", "значок"). Добавляй только marketplace, раздел и метрику, которые пользователь явно назвал или которые однозначно следуют из вопроса.
- НЕ дописывай в search_query догадки о причинах, соседние темы или термины, которых пользователь не упоминал (например не добавляй "ошибки загрузки", "себестоимость", "фильтры" к вопросу про значок у товара). Лишние термины перекашивают векторный поиск и вытесняют правильный фрагмент.
...
async def plan_query(
    ...
    *,
    ...
    recent — недавний диалог {last_user_text, last_bot_answer} для разрешения follow-up.
    """
    payload = {
