...
Продолжение диалога (follow-up). После каждого содержательного (RAG-обоснованного) ответа `processor.py` добавляет пару «вопрос-ответ» в скользящее окно истории в SQLite (`recent_exchanges_json`): хранится не более `RAG_HISTORY_MAX_PAIRS` последних пар (по умолчанию 3), ответ бота обрезается до `RAG_HISTORY_ANSWER_CHARS` символов (по умолчанию 400), чтобы не раздувать ввод planner. Разговорные реплики и уточнения, отвеченные из памяти (ветка `is_conversational`), в историю НЕ пишутся — поэтому «ок»/«спасибо» между вопросом и уточнением не вытесняют последнюю реальную тему. При следующей обработке, если цепочка диалога жива (есть `last_response_id`), это окно передаётся в planner как `recent_dialogue` (список пар, самая свежая последней). Planner использует его, чтобы разрешить отсылки («это», «здесь», «т.е.», «а если») и понять, что текущее сообщение — уточнение к предыдущей теме, а не новый вопрос. Дальше planner выбирает путь: (1) follow-up, на который можно ответить из предыдущего ответа без новых фактов → `ready` с пустым `search_query` (как `is_conversational`), ассистент отвечает из контекста по `last_response_id`; (2) follow-up за новыми фактами → `ready` с `search_query`, в который обязательно включается тема предыдущего вопроса (например «выкупаемость», а не только «заказы в пути»), чтобы поиск не ушёл в чужой раздел. `recent_dialogue` не передаётся, если цепочка сброшена (после перевода на оператора или с нуля) — тогда история уже неактуальна. Полную историю диалога planner и reranker не получают: им нужен сфокусированный сигнал, а вся переписка хранится на стороне ассистента через `last_response_id`.

Если данных достаточно, planner возвращает нормализованный `search_query`. Запрос держится близко к формулировке пользователя: planner сохраняет ключевые слова из вопроса (например «восклицательный знак», «иконка») и добавляет только явно названные маркетплейс/раздел/метрику, но НЕ дописывает догадки о причинах и соседние термины — лишние слова перекашивают векторный поиск и вытесняют правильный фрагмент. `processor.py` отправляет в `vector-base` именно этот запрос, запрашивает `RAG_SEARCH_TOP_K` кандидатов (по умолчанию 10, чтобы нужный чанк не отрезался на границе выдачи), затем `reranker.py` отбирает до `RAG_RERANK_KEEP` чанков для финального ответа. Reranker всегда возвращает 1–3 самых релевантных по теме чанка, если они есть, и отдельно флагом `enough_context` оценивает полноту: `true` — фрагменты дают прямой ответ, `false` — контекст частичный. При `enough_context=false`, но непустом наборе чанков `processor.py` работает в мягком режиме: передаёт частичный контекст ассистенту с пометкой о неполноте, и решение «звать оператора» принимает сам ассистент через `transfer_to_operator`. На оператора запрос переводится сразу только когда reranker не нашёл ни одного тематического чанка (пустой набор). Planner и reranker являются внутренними stateless Responses-вызовами и не используют `last_response_id`.
