...
Отдельное правило для вопросов про расхождение/несовпадение данных («не сходятся», «не совпадает», «расхождение», «сверка»): причины и отчёты для сверки различаются по маркетплейсам, поэтому если маркетплейс не назван (и не указано, что сравнение идёт с данными внутри Sellerdata), planner возвращает `need_clarification` и спрашивает «С чем именно не сходятся данные — с Wildberries, с Ozon или с данными в Sellerdata?». Если маркетплейс уже назван в вопросе или в pending-уточнении — planner возвращает `ready` и не переспрашивает.
