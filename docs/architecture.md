...
Если данных достаточно, planner возвращает нормализованный `search_query`. `processor.py` отправляет в `vector-base` именно этот запрос, запрашивает `RAG_SEARCH_TOP_K` кандидатов, затем `reranker.py` выбирает до `RAG_RERANK_KEEP` чанков для финального ответа. Planner и reranker являются внутренними stateless Responses-вызовами и не используют `last_response_id`.
