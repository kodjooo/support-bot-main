...
Продолжение диалога (follow-up). После каждого успешного ответа `processor.py` сохраняет в SQLite последнюю пару «вопрос-ответ» (`last_user_text`, `last_bot_answer`). При следующей обработке, если цепочка диалога жива (есть `last_response_id`), эта пара передаётся в planner как `recent_dialogue`. Planner использует её, чтобы разрешить отсылки («это», «здесь», «т.е.», «а если») и понять, что текущее сообщение — уточнение к предыдущей теме, а не новый вопрос. Дальше planner выбирает один из путей: (1) follow-up, на который можно ответить из предыдущего ответа без новых фактов → `ready` с пустым `search_query` (как `is_conversational`), ассистент отвечает из контекста по `last_response_id`; (2) follow-up за новыми фактами → `ready` с `search_query`, в который обязательно включается тема предыдущего вопроса (например «выкупаемость», а не только «заказы в пути»), чтобы поиск не ушёл в чужой раздел. `recent_dialogue` не передаётся, если цепочка сброшена (после перевода на оператора или с нуля) — тогда прошлая пара уже неактуальна.
